...
### Favicon
faviconThe favicon has been produced in all relevant file formats.
...
### The Works-section
  ...
  + The images of the art-pieces zooms to a larger size when hovered
  + The user can click an art-piece if he want to see a larger display of the art piece.
...
### The Contact-section with form
  ...
  + An the bright orange submit-button acts here as a "call-to-action" button, the action being to reach out to the artist.
...
### Typography
A font-matching-tool has been used to test combinations of "header-fonts" and "paragraph-fonts" however its advice was ultimately discarded for a "gutfeel" to pick the single font "Open Sans Regular 400" with "Montserrat" as a fallback font and sans-serif as the fallback to that.
...
+ CSS using the [Jigsaw-the official validator for CSS-code}(https://jigsaw.w3.org/css-validator/)
...
+ The reason why the Accesibility score is not perfect is because Lighthouse comes down on our countious design choise of a light grey (instead of a pure white) font colour on a black background, thus not reaching optimal contrast.
